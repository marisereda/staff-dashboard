...
async function main() {
  ...
  const newEmployeesPromises = employees.map(async employee => await prisma.employee.create({ data: employee }));
  ...
  const newEmployersPromises = employers.map(async employer => await prisma.employer.create({ data: employer }));
